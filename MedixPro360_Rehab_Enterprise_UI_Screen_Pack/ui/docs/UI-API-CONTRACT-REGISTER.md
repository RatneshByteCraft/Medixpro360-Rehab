...
| Capture consent | `captureConsent` | `clientId`, caregiver/scope reference | `consent.capture` | Type, scope and effective date required | Consent capture audit | `POST /api/consents` |
| Revoke consent | `revokeConsent` | `consentId`, `clientId` | `consent.revoke` | Active -> Revoked; reason required; no deletion | Consent revocation audit | `POST /api/consents/{id}/revoke` |
